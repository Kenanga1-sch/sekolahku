...
func (r *StaffProfileRepository) GetProfiles(search string) ([]models.StaffProfile, error) {
	...
	for rows.Next() {
		...
		if deg.Valid { p.Degree = &deg.String }
		if photo.Valid { p.PhotoUrl = &photo.String }
		if nip.Valid { p.NIP = &nip.String }
		if quote.Valid { p.Quote = &quote.String }
		
		cTime := ToTime(crAt); p.CreatedAt = &cTime
		uTime := ToTime(upAt); p.UpdatedAt = &uTime
...
func (r *StaffProfileRepository) Update(id string, p models.StaffProfile) error {
	...
	query := `
		...
		WHERE id = ?
	`
	_, err := r.DB.Exec(query, p.Name, p.Degree, p.Position, p.Category, p.PhotoUrl, p.NIP, p.Quote, p.DisplayOrder, p.IsActive, now, id)
	return err
}

func (r *StaffProfileRepository) Delete(id string) error {
	_, err := r.DB.Exec("DELETE FROM staff_profiles WHERE id = ?", id)
	return err
}
