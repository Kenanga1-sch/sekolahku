...
func (r *SettingRepository) GetSettings() (*models.SchoolSettings, error) {
	...
	if npsn.Valid { s.SchoolNPSN = &npsn.String }
	...
	if web.Valid { s.SchoolWebsite = &web.String }
	if logo.Valid { s.SchoolLogo = &logo.String }
	if lat.Valid { s.SchoolLat = &lat.Float64 }
	if lng.Valid { s.SchoolLng = &lng.Float64 }
	if dist.Valid { s.MaxDistanceKM = &dist.Float64 }
	if pName.Valid { s.PrincipalName = &pName.String }
	if pNip.Valid { s.PrincipalNIP = &pNip.String }
	if treasurerId.Valid { s.SavingsTreasurerID = &treasurerId.String }
	if vision.Valid { s.SchoolVision = &vision.String }
	if mission.Valid { s.SchoolMission = &mission.String }
	if indicators.Valid { s.SchoolIndicators = &indicators.String }
	if historyTimeline.Valid { s.SchoolHistoryTimeline = &historyTimeline.String }
	if historyAchievements.Valid { s.SchoolHistoryAchievements = &historyAchievements.String }
	if curriculum.Valid { s.SchoolCurriculum = &curriculum.String }
	if extras.Valid { s.SchoolExtracurriculars = &extras.String }
