...
export default function InventarisPage() {
    ...
    const [consumableStats, setConsumableStats] = useState<any>(null);
    ...
    async function loadStats() {
        try {
            // Fetch Assets Stats
            const resAssets = await fetch("/api/inventory/assets/dashboard");
            if (resAssets.ok) {
                const data = await resAssets.json();
                setStats(data);
            }
            
            // Fetch Consumable Stats (Only if admin)
            // Note: We use the direct role check here for logic, as loadStats is client-side async anyway
            if (["superadmin", "admin"].includes(user?.role || "")) {
                const resConsumables = await fetch("/api/inventory/stats");
                if (resConsumables.ok) {
                    const data = await resConsumables.json();
                    setConsumableStats(data);
                }
            }
    ...
    }, [isMounted, user?.role]); // Re-run if role changes
    ...
    const statCards = [
        ...
        },
        {
            ...
            icon: AlertTriangle,
            color: (stats?.itemsDamaged || 0) + (stats?.itemsLost || 0) > 0 ? "text-red-500" : "text-gray-400",
            bgColor: (stats?.itemsDamaged || 0) + (stats?.itemsLost || 0) > 0 ? "bg-red-500/10" : "bg-gray-500/10",
            gradient: "from-red-500 to-pink-500",
            desc: "Perlu perhatian",
            alert: (stats?.itemsDamaged || 0) + (stats?.itemsLost || 0) > 0,
        },
    ];
        ...
        <div className="space-y-8">
            ...
            <Tabs defaultValue="aset" className="space-y-6">
                ...
                <TabsContent value="aset" className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    {/* Damaged/Lost Alert */}
                    {stats && (stats.itemsDamaged > 0 || stats.itemsLost > 0) && (
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-medium text-orange-700 dark:text-orange-400">
                                            {stats.itemsDamaged + stats.itemsLost} barang perlu perhatian
                                        </p>
                                        <p className="text-sm text-orange-600/70 dark:text-orange-400/70">
                                            {stats.itemsDamaged} rusak, {stats.itemsLost} hilang
                                        </p>
                                    </div>
                ...
                {isAdmin && (
                    <TabsContent value="atk" className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{consumableStats?.totalItems || 0}</div>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{formatCurrency(consumableStats?.totalValue || 0)}</div>
                            <p className="text-xs text-muted-foreground mt-1">
                        ...
                        <Card className={`hover:shadow-lg transition-all duration-300 border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm ${consumableStats?.lowStockCount ? 'border-amber-500/50 dark:border-amber-500/50' : ''}`}>
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-sm font-medium text-muted-foreground">
                            Stok Menipis
                            </CardTitle>
                            <AlertTriangle className={`h-4 w-4 ${consumableStats?.lowStockCount ? 'text-amber-500' : 'text-zinc-500'}`} />
                        </CardHeader>
                        <CardContent>
                            <div className={`text-2xl font-bold ${consumableStats?.lowStockCount ? 'text-amber-600 dark:text-amber-500' : ''}`}>
                            {consumableStats?.lowStockCount || 0}
                            </div>
                            <p className="text-xs text-muted-foreground mt-1">
                            Barang perlu restock segera
                            </p>
                            {consumableStats?.lowStockCount ? (
                                <Link href="/inventaris/stok" className="text-xs text-amber-600 hover:underline mt-2 inline-block">
                                    Lihat detail →
                                </Link>
                            ) : null}
                        </CardContent>
                    ...
                    <Card className="border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm">
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                {consumableStats?.recentTransactions?.length === 0 ? (
                                <div className="text-center py-8 text-muted-foreground">
                                    Belum ada transaksi tercatat.
                                </div>
                                ) : (
                                consumableStats?.recentTransactions?.map((trx: any) => (
                                    <div key={trx.id} className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-white/5 border border-zinc-100 dark:border-white/5 shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {format(new Date(trx.date), "dd MMM yyyy", { locale: id })} • {trx.user?.name}
                                            </p>
                                            </div>
                                        </div>
