...
export default function SetoranDetailPage() {
    ...
    const setoranId = searchParams.get('id') as string;
    ...
    const fetchSetoran = useCallback(async () => {
        setIsLoading(true);
        try {
            const data: any = await goGet(`/api/tabungan/setoran/detail?id=${setoranId}`);
            if (data.success) {
    ...
    const handleSaveEdit = async (txId: string) => {
        ...
        try {
            const data: any = await goPut(`/api/tabungan/setoran/detail?id=${setoranId}/transaksi/${txId}`, {
    ...
    const handleResubmit = async () => {
        setIsSubmitting(true);
        try {
            const data: any = await goPut(`/api/tabungan/setoran/detail?id=${setoranId}`, { catatan: resubmitCatatan || null });
            if (data.success) {
