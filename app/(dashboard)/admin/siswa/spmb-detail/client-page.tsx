...
import { goGet, goPatch, goDelete } from "@/lib/api-client";
...
interface RegistrantDetail {
    ...
    documents: string | (string | DocumentItem)[];
...
export default function RegistrantDetailPage() {
    ...
    const fetchRegistrant = useCallback(async () => {
        try {
            const data: any = await goGet(`/api/spmb/registrants/${id}`);
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await goPatch(`/api/spmb/registrants/${id}`, {
    ...
    const handleDelete = async () => {
        try {
            await goDelete(`/api/spmb/registrants/${id}`);
            router.push("/admin/siswa?tab=spmb");
        } catch (error) {
