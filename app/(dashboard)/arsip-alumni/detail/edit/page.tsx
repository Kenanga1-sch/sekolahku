...
export default function EditAlumniPage() {
  ...
  const onSubmit = async (data: AlumniFormData) => {
    ...
    try {
      const result: any = await goPut(`/api/alumni/${searchParams.get('id')}`, data);

      if (result.error) {
        throw new Error(result.error || "Failed to update alumni");
      }

      router.push(`/arsip-alumni/detail?id=${searchParams.get('id')}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center gap-4">
        <Link href={`/arsip-alumni/detail?id=${searchParams.get('id')}`}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="flex justify-center">
            <PhotoUpload
              alumniId={searchParams.get('id') as string}
